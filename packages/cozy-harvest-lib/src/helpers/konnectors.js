...
/**
 * Parse base directory, based on given folders object.
 * For example, it will parse `$administrative` to the given value passed in
 ...
 */
const parseBaseDir = (baseDir, folders = {}) => {
  ...
  return folders[baseDir.slice(1)] || DEFAULT_LOCALIZED_BAR_DIR
}

/**
 * Parse the given folder path using the given `variables` object.
 * Variables are injected in `buildFolderPath` function. Available variable are:
 * $konnector (konnector name) and $account (account label, i.e. id or name)
 * @param  {String} path      Path to parse
 * @param  {Object} variables Object mapping variable to actual values
 * @return {String}           Parsed path
 */
const parseFolderPath = (path, variables = {}) => {
...
export const buildFolderPath = (konnector, account, folders = {}) => {
  ...
  // example.
  const parsedBaseDir = parseBaseDir(baseDir, folders)
  const parsedPath = parseFolderPath(sanitizedPath, {
    // When adding a new allowed variable here, please keep documentation
    // of `parseFolderPath` function up to date.
    konnector: konnector.name,
    account: accounts.getLabel(account)
  })

  return `/${parsedBaseDir}/${parsedPath}`
}
